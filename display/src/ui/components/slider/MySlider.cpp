...
lv_obj_t* create_custom_slider(lv_obj_t* parent, int32_t min, int32_t max, int32_t width, lv_event_cb_t event_cb) {
    ...
    lv_obj_set_ext_click_area(slider, 20); // Extend touch area
    ...
    // Background radius
    lv_obj_set_style_radius(slider, 20, LV_PART_MAIN); // Fully rounded
    
    // Indicator (Filled bar)
    lv_obj_set_style_bg_color(slider, lv_color_hex(0x00FF00), LV_PART_INDICATOR); // Green by default

    // Knob (Ball) - Make it large
